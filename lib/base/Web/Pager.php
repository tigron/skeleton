...
class Web_Pager {
	...
	public function __construct($classname = null, $sort = 'id', $direction = 'ASC', $page = 1, $extra_conditions = array(), $all = false) {
		...
		if (isset($_GET['extra_conditions'])) {
			$this->options['extra_conditions'] = unserialize(base64_decode($_GET['extra_conditions']));
		} else {
	...
	private function generate_links() {
		...
		for ($i = $first_page_link; $i<=$last_page_link; $i++) {
			...
			if ($i == $this->options['page']) {
				...
				$links .= '<a href="' . $_SERVER['REDIRECT_URL'] . '?page=' . $i . '&sort=' . $this->options['sort'] . $search . '&direction=' . $this->options['direction'] . '&extra_conditions=' . urlencode(base64_encode(serialize($this->options['extra_conditions']))) . '">' . $i . '</a>';
			}
		}
		if ($first_page_link > 1) {
			$links = '<a href="' . $_SERVER['REDIRECT_URL'] . '?page=1&sort=' . $this->options['sort'] . $search . '&direction=' . $this->options['direction'] . '&extra_conditions=' . urlencode(base64_encode(serialize($this->options['extra_conditions']))) . '">[1]</a>&nbsp;&nbsp;&nbsp;' . $links;
		}
		if ($last_page_link < $pages) {
